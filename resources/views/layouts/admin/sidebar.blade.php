    ...
    <div class="sidebar">
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                @php
                    $roles = App\RoleUser::getMenu()->where('users.id', '=', Auth::user()->id)->orderBy('rank')->distinct()->get();
                @endphp
                @foreach($roles as $role)
                    <li class="nav-item">
                        <router-link tag="a" to="/{{$role->url}}"  class="nav-link">
                            <i class="nav-icon {{$role->icon}}"></i>
                            <p>{{$role->name}}</p>
                        </router-link>
                    </li>
                @endforeach
